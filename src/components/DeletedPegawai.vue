...
export default {
    ...
    created() {
        ...
        axios.get("http://luxinoire.com/api/deletedPegawai").then(response => {
    ...
    methods: {
        restore(item) {
            ...
            console.log(temp["id"])
            confirm('Kembalikan Item?') && this.pegawai.splice(index, 1)&&
            axios
            .put("http://luxinoire.com/api/updatePegawai/"+item["id"], {
        ...
        deleteItem(item) {
          ...
          var temp = Object.assign({}, item)
          console.log(temp["id"])
          confirm('Hapus Permanen Item?') && this.pegawai.splice(index, 1) &&
