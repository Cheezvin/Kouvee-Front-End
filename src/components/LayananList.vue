    ...
    <v-data-iterator
     ...
     <template v-slot:header>
       <v-img
          ...
          to="/layanan"
        />
        <v-toolbar-title style="color: white;" class="text-center display-3 pb-5">Layanan Kami</v-toolbar-title>
        <v-toolbar
          class="mb-1"
          elevation="0"
          color="#222831"
      ...
      <template v-slot:default="props" >
        <v-row>
          <v-col v-for="p in props.items" :key="p.name" cols="4">
             <v-card style="border-radius:10px;" color="#ececec"> 
             <v-img class="text-center" height="200" widht="200" :src="p.gambar"></v-img>
              <v-card-subtitle class="font-weight-black text-truncate">{{p.nama}}</v-card-subtitle>
              <v-card-text class="font-weight-black title">
                <div>
                  IDR {{p.harga}}
                </div>
              </v-card-text>
